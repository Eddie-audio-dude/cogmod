...
2. This is a follow-up to the previous exercise.  In applications of the EBRW to experiments that vary the number of items, it is typical to assume that the "criterion" varies linearly with list length [@NosofskyEtAl2011; @NosofskyEtAl2014].  Modify our `ebrw_nll` function to accommodate this version of the model.  The chunk of code below gives a suggestion about how this might be done in such a way that preserves the functionality of the code we used throughout this chapter by keeping the "criterion" parameter and adding a "criterion_slope" parameter.
```{r eval = FALSE}
#| code-fold: show

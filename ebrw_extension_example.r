...
for (i in 1:(nrow(stim_reps) - 1)) {
    for (j in (i + 1):nrow(stim_reps)) {
        for (k in 1:nrow(stim_reps)) {
            study_items <- rbind(
                study_items,
                c(i, j)
            )
...
fit <- nlminb(
    ...
    objective = extended_ebrw_nll,           # Tell R the name of the function to optimize
    lower = lower,                  # The lower bounds on each parameter
    upper = upper,                  # The upper bounds on each parameter
    stim_reps = stim_reps,      # The coordinates of each stimulus
